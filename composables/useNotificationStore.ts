...
export const useNotificationStore = defineStore('notification', () => {
  const data = ref<MsgDataType>(msgData);
  const unreadMsg = computed(() => data.value.filter((item: NotificationItem) => item.status));
  const readMsg = computed(() => data.value.filter((item: NotificationItem) => !item.status));
